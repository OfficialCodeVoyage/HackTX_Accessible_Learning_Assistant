...
// Update the Question type to match the JSON structure
type Question = {
  question: string;
  choices: {
    A: string;
...
export function MarioRun({ questions, onClose }: MarioRunProps) {
  console.log('MarioRun props:', { questions, onClose });
  ...
  const initializeObstacles = useCallback(() => {
    ...
    if (currentQuestionIndex < questions.length) {
      ...
      const options = Object.values(currentQuestion.choices);
      gameStateRef.current.obstacles = options.map((option, index) => ({
  ...
  const updateGameState = useCallback(() => {
    ...
    gameState.obstacles = gameState.obstacles.filter(obstacle => {
      if (
        ...
        if (obstacle.text === questions[gameState.currentQuestionIndex].correct_answer) {
          gameState.score += 100;
  ...
  const draw = useCallback(() => {
    ...
    if (currentQuestionIndex < questions.length) {
      ctx.textAlign = 'center';
      ctx.fillText(questions[currentQuestionIndex].question, CANVAS_WIDTH / 1, 70);
    }
    if (gameOver || gameWon) {
